...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Plot spectrum responses by normalized log probability difference."
        )
    )
    parser.add_argument(
        "input_path",
...
def load_plot_rows(path, metric):
    ...
    rows = [row for row in rows if isinstance(row.get(metric), (int, float))]
    if not rows:
        raise ValueError(f"No rows with numeric {metric!r} found in {path}")
...
def plot(rows, metric, output_path, wrap_width):
    y_values = label_lanes(rows, metric)
    fig_height = max(6.0, 4.0 + 0.8 * len(set(y_values)))
    fig, ax = plt.subplots(figsize=(18, fig_height))

    x_values = [row[metric] for row in rows]
    ...
    ax.set_xlabel("Normalized log P difference: owl system minus neutral system")
    ax.set_title("Spectrum Responses", loc="left", pad=18)
